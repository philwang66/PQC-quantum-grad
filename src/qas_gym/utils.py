...
def get_default_gates(n_qubits, gateset_option='default')  -> List[QuantumGateBase]:
    ...
    for i in range(n_qubits):
        if gateset_option == 'default':
        ...
        elif gateset_option == 'PauliSame2':
            ...
            for j in range(i+1,n_qubits):
                ...
                gates += [
                    ...
                    PauliRotation(target_list, [2, 2], np.pi / 4.) , 
                    PauliRotation(target_list, [3, 3], np.pi / 4.) ,
                    PauliRotation(target_list, [1, 2], np.pi / 4.) ,
                    PauliRotation(target_list, [1, 3], np.pi / 4.) , 
                    PauliRotation(target_list, [2, 1], np.pi / 4.) ,
                    PauliRotation(target_list, [2, 3], np.pi / 4.) , 
                    PauliRotation(target_list, [3, 1], np.pi / 4.) ,
                    PauliRotation(target_list, [3, 2], np.pi / 4.)                   
                ]
        elif gateset_option == 'VQE':
            # according to CRLQAS-ICLR2024
            for j in range(i+1,n_qubits):
                target_list = [i, j]
                gates += [
                    PauliRotation(target_list, [3, 3], np.pi / 4.)               #RZZ
                ]
                for k1 in range(j+1, n_qubits):
                    for k2 in range(k1+1, n_qubits):
                        gates += [
                            PauliRotation([i,j,k1,k2], [2, 1, 1, 2], np.pi / 4.) ,   #RYXXY
                            PauliRotation([i,j,k1,k2], [1, 1, 2, 2], np.pi / 4.) ,   #RXXYY   
                        ]
        else:
...
def add_spaces_to_operators(operator_string):
    """Add space between operator and number"""
    return re.sub(r'([XYZ])(\d+)', r'\1 \2', operator_string)

def get_default_observables(n_qubits, obs_option='default', open_fermion_str=None) -> List[Observable]:
    observables = []
    for qubit in range(n_qubits):
        ...
        if obs_option == 'TwoPauliSame':
            for qubit2 in range(qubit, n_qubits):
                XXobs = Observable(n_qubits)
                XXobs.add_operator(1.0, "X {} X {}".format(qubit, qubit2))
                YYobs = Observable(n_qubits)
                YYobs.add_operator(1.0, "Y {} Y {}".format(qubit, qubit2))
                ZZobs = Observable(n_qubits)
                ZZobs.add_operator(1.0, "Z {} Z {}".format(qubit, qubit2))
                observables += [
                    XXobs, YYobs, ZZobs,
        ]
        if obs_option == 'OpenFermion':
            # print('Using Obsv from openFermion')
            # operator = create_quantum_operator_from_openfermion_text(open_fermion_str)
            # print(operator.get_qubit_count())
            # for t in range(operator.get_term_count()):
            #     observables += operator.get_term(t)
            # observables =[]
            pattern = r'\[([^\]]+)\]'
            matches = re.findall(pattern, open_fermion_str)

            # Filter out empty strings and get unique operators
            operators = [add_spaces_to_operators(match) for match in matches if match.strip()]
            for op in operators:
                obs = Observable(n_qubits)
                obs.add_operator(1.0, op)
                observables += [obs]
    return observables
