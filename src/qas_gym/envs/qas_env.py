...
from qas_gym.envs.vqc import get_fidelity_pc, calculate_fidelity
...
class QuantumArchSearchEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        return self._get_obs_initial(seed), dict(initial = self.initial, target = self.target)
    ...
    def step(self, action):
        ...
        return observation, reward, terminal, True, info
...
class QuantumArchSearchGeneralEnv(gym.Env):
    metadata = {'render.modes': ['ansi', 'human']}
    ...
    def __init__(
        ...
        super(QuantumArchSearchGeneralEnv, self).__init__()
        ...
        self.observation_space = spaces.Box(low=-1.,
                                            high=1.,
                                            shape=(2*len(state_observables), ))
        self.action_space = spaces.Discrete(n=len(action_gates))
        self.optim_alg = 'COBYLA' #'Nelder-Mead'
        self.global_iters = 1000
        # seed = self.seed()
        # print(type(seed[0]))
        self.initial = QuantumState(self.num_qubits)
        if initial is not None:
            self.initial.load(initial)
        else:
            self.initial.set_Haar_random_state()
        # seed = self.seed(seed[0])

        self.target = QuantumState(self.num_qubits)
        if target is not None:
            self.target.load(target)
        else:
            self.target.set_Haar_random_state()
        self.fidelity_prev = calculate_fidelity(self.initial.get_vector(), self.target.get_vector())


    def __str__(self):
        desc = 'QuantumArchSearchGeneral-v0('
        # desc += '{}={}, '.format('Qubits', len(self.qubits))
        desc += '{}={}, '.format('Initial', self.initial)
        desc += '{}={}, '.format('Target', self.target)
        desc += '{}=[{}], '.format(
    ...
    def reset(self, seed=None, options=None):
        ...
        if options=='random':
            self.initial = QuantumState(self.num_qubits)
            self.target = QuantumState(self.num_qubits)
            self.initial.set_Haar_random_state(seed)
            self.target.set_Haar_random_state(seed+99999)
        # else:
        #     # assume options are dictionary of initial and target
        #     self.initial.load(options['initial'])
        #     self.target.load(options['target'])

        obs = [o.get_expectation_value(self.initial) for o in self.state_observables]
        obs_diff = [o.get_expectation_value(self.target)-o.get_expectation_value(self.initial) for o in self.state_observables]
        obs.extend(obs_diff)
        # obs.extend([o.get_expectation_value(self.target) for o in self.state_observables])
        return np.array(obs).real, dict(initial = self.initial, target = self.target)
    ...
    def _get_obs_target(self, seed=None):
        # print(self.initial)
        state = QuantumState(self.num_qubits)
        if self.target is not None: # and seed is None
            state = self.target
        else:
            state = QuantumState(self.num_qubits)
            state.set_Haar_random_state(seed)
    ...
    def _get_fidelity_qulacs(self, circuit):
        ...
        simulator = QuantumCircuitSimulator(circuit, state)
        simulator.simulate()
        return calculate_fidelity(state.get_vector(), self.target.get_vector())
        ...
        if parameter_count_qulacs > 0:
            ...
            if list(which_angles):
                ...
                result_min_qulacs = optimize.minimize(get_fidelity_pc, x0=x0[which_angles],
                                                                args=(circuit,
                                                                    self.num_qubits,
                                                                    self.target.get_vector(),
                                                                    self.initial.get_vector(),
                                                                    which_angles),
                                                                method=method,
                                                                bounds=bnds,
                                                                options={'maxiter':self.global_iters})
                ...
                if  result_min_qulacs['success']:
                    ...
                    result = -get_fidelity_pc(x0, circuit, self.num_qubits, self.target.get_vector(),self.initial.get_vector())
        else:
            result = self._get_fidelity_qulacs(circuit)
            x0 = None
    ...
    def step(self, action):
        ...
        # compute observation
        observation = self._get_obs()
        # observation = np.concatenate((observation, self._get_obs_target()), axis=None)
        observation = np.concatenate((observation, self._get_obs_target()-observation), axis=None)
        ...
        # print("Parameters: ", thetas)
        # print("Fidelity:", fidelity)
        if action_gate.get_name() in ('Pauli-rotation', 'CNOT', 'CZ'):
            reward_penalty = self.reward_penalty*10
        else:
            reward_penalty = self.reward_penalty
        # compute reward
        if fidelity >= self.fidelity_threshold:
            reward = 10.0 - reward_penalty
        elif fidelity < self.fidelity_threshold and self.ansatz.get_gate_count() >= self.max_timesteps:
            reward = -5.0
        else:
            reward = max((fidelity-fidelity_prev)/(self.fidelity_threshold - fidelity_prev), -1.0) - reward_penalty
